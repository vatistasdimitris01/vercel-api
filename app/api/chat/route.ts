...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      groundingQueries = (formData.getAll("groundingQueries") as string[]).filter(Boolean);
    } else {
      const body = await req.json();
      input = body.input || "";
      uploadedUrls = (body.urls as string[]) || [];
      groundingQueries = (body.groundingQueries as string[]) || [];
    }
    ...
    const config = groundingQueries.length
      ? { tools: [{ googleSearch: { queries: groundingQueries } }] }
      : {};
